...
var challengeCrawlers = exports.challengeCrawlers = function(ua, result) {
  ...
  if (ua.indexOf('compatible; Yandex') >= 0) {
    updateMap(result, dataset.get('Yandex'));
    return true;
  }
  if (ua.indexOf('DuckDuckBot') >= 0 ||
      ua.indexOf('DuckDuckGo') >= 0 ||
      ua.indexOf('ddg_android') >= 0) {
    updateMap(result, dataset.get('DuckDuckGo'));
    return true;
  }
